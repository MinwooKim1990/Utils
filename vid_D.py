...
listen_port = 8443    # HTTPS 포트 (변경: 8443)
...
# Cheroot WSGI 서버 설정 (HTTPS)
server_args = {
    "bind_addr": (listen_ip, listen_port),
    "wsgi_app": app,
    "numthreads": 12, # 워커 스레드 수 조절 (7950X3D, 2개 서버 동시 실행 고려)
}
...
print("=" * 50)
print(f"간단한 WebDAV 서버 (HTTPS) 시작 중...")
print(f"접속 주소: https://{pc_ip_address}:{listen_port}/")
...
print(f"  - 경로: / (또는 비워둠)")
print(f"  - HTTPS: 활성화 (켜기)")  # 중요! HTTPS 활성화
print("-" * 50)
