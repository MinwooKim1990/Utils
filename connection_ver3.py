...
def upload_file():
    ...
    if drive not in ALLOWED_DRIVES:
        drive = 'E:/'
    # base_dir = drive # 이 줄은 사용되지 않으므로 제거해도 됩니다.
    rel_path = request.args.get('path', '')
    save_dir = os.path.join(drive, rel_path) # full_path 대신 save_dir 사용
...
# ----------- Gunicorn + gevent 권장 ----------- 
# gunicorn -w 4 -k gevent -b 0.0.0.0:443 --certfile=cert.pem --keyfile=key.pem Utils.connection_ver3:app
